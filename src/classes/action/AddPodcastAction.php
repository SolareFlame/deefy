...
class AddPodcastAction extends Action {
    ...
    public function executePost(): string
    {
            if (substr($_FILES['userfile']['name'], -4) === '.mp3') {
                if ($_FILES['userfile']['type'] === 'audio/mpeg') {
                    ...
                    $track = new tracks\PodcastTrack($uuid, $title, $genre, $duration, $filename, $artists, $date);

                    //ajouter la track à la DB
                    $instance = DeefyRepository::getInstance();
                    $instance->addPodcast($track);

                    if (!move_uploaded_file($_FILES['userfile']['tmp_name'], $filename)) {
                        return "Erreur lors de l'upload du fichier";
                    }
                    return '<a href="?action=add-podcast">Ajouter encore un podcast</a>';
                } else {
    ...
    {
            return <<<FORM
                <form action='index.php?action=add-podcast' method='POST' enctype='multipart/form-data'>
                <input type="file" name="userfile" accept="audio/*" required>
                <input type='submit' value='Ajouter un podcast'>
                </form>
            FORM;
    }
}